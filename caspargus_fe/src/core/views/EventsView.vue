...
<template>
  <div class="absolute top-0 left-0 size-full bg-slate-600 pt-4">
    <div class="relative h-40 w-1/2 left-1/2 -translate-x-1/2">
      <div class="text-white text-2xl font-bold">EVENTS</div>
      <br />
      ...
      <ul class="list bg-base-100 rounded-box shadow-md mb-6">
        <li class="p-4 text-xs opacity-60 tracking-wide flenter">
          ...
          <template v-else> Please create an event </template>
        </li>

        <li
          class="list-row flex flex-row"
          v-for="event in eventStore.eventsSummaries"
          :key="event._id"
        >
          <div class="grow">
            <div>{{ event.name }}</div>
          ...
          <button
            ...
            @click="$router.push(`/configurator/${event._id}`)"
          >
      ...
      <div class="flenter">
        <input
          ...
          v-model="eventName"
        />
        <button class="btn btn-primary ml-2" @click="createEventSummary">
          Create Event
        </button>
      </div>
    </div>
    ...
    <div
      v-if="socketStore.error"
      class="absolute bottom-0 left-0 w-full bg-red-500 text-white p-2 text-center"
    >
